...
elseif (Config.FRAMEWORK == 'esx-old') then
    ...
    print('^6[^3Corey-PostalJob^6]^0 Unsupported Framework detected!')
end

local PAY_MULTIPLIER = Config.PAY_MULTIPLIER

RegisterServerEvent("Corey-PostalJob:server:start:job", function()
    local source = source
    ...
    if dist > 25 then
        print(("^8[CheatFlag]^0 Corey-PostalJob:server:start:job: %s(%s) Tried to start the job while %.2f meters away from marker"):format(GetPlayerName(source), GetPlayerIdentifier(source, 0), dist))
        return
    end
    ...
    TriggerEvent("Corey-PostalJob:server:log", { source = source, type = "success", message = t('started_postal_job')})
end)

RegisterServerEvent("Corey-PostalJob:server:end:job", function()
    if not currentWorkers[source] then return end

    currentWorkers[source] = nil
    TriggerEvent("Corey-PostalJob:server:log", { source = source, type = "success", message = t('ended_shift')})
end)

---@param positionSet {startLocation: vector3, middleLocation: vector3, endLocation: vector3}
RegisterServerEvent('Corey-PostalJob:server:compensateDelivery', function(positionSet)
    local source = source
    local data = currentWorkers[source]

    if not data then
        print(("^8[CheatFlag]^0 Corey-PostalJob:server:compensateDelivery: %s(%s) Tried to deliver packages without starting the job"):format(GetPlayerName(source), GetPlayerIdentifier(source, 0)))
        return
    end

    local time = os.time() - data.lastPoint
    if os.time() - data.lastPoint < 30 then
        print(("^8[CheatFlag]^0 Corey-PostalJob:server:compensateDelivery: %s(%s) Tried to deliver packages while the last package was %s seconds ago"):format(GetPlayerName(source), GetPlayerIdentifier(source, 0), time))
        return
    end

    if #Config.POSTAL_DROP_OFF_PACKAGE - data.takenPoints < 0 then
        print(("^8[CheatFlag]^0 Corey-PostalJob:server:compensateDelivery: %s(%s) Tried to deliver packages while all point already taken?"):format(GetPlayerName(source), GetPlayerIdentifier(source, 0)))
        return
    end

    if not isNearAnyDeliverPoint(source) then
        print(("^8[CheatFlag]^0 Corey-PostalJob:server:compensateDelivery: %s(%s) Tried to deliver packages while not near any deliverpoint"):format(GetPlayerName(source), GetPlayerIdentifier(source, 0)))
        return
    end
    ...
    TriggerClientEvent('Corey-PostalJob:client:notifyPlayer', source, compensationMessage, 'success')
    ...
    data.takenPoints += 1
    TriggerEvent("Corey-PostalJob:server:log", { source = source, type = "success", message = t('delivered_a_package')})
end)
...
AddEventHandler('Corey-PostalJob:server:log', function(data)
